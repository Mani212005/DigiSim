 ...
 * evaluator — the hook and the circuit-analysis tools (truth tables) both call
 * it so gate logic never gets duplicated.
 *
...
export const useLogicSimulation = (): { simulateCircuit: SimulateCircuit } => {
  const simulateCircuit = useCallback<SimulateCircuit>(
    (currentNodes, currentEdges) => runSimulation(currentNodes, currentEdges),
    []
  );
